...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber out_virtuose_status = n.subscribe<virtuose::out_virtuose_status>("out_thumb_status", 1, out_virtuose_statusCB);
  ros::Subscriber out_virtuose_pose = n.subscribe<virtuose::out_virtuose_pose>("out_thumb_pose", 1, out_virtuose_poseCB);
  ros::Publisher in_virtuose_force = n.advertise<virtuose::in_virtuose_force>("in_thumb_force", 1);
  ...
  ros::ServiceClient virtuose_impedance = n.serviceClient<virtuose::virtuose_impedance>("thumb_impedance");
  ...
  while(ros::ok())
  {
    ...
    in_virtuose_force.publish(force); //####to publish force
    ctr ++;
